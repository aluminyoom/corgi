const LINK_ID = "corgi-cc-link";
const SETTINGS_HREF = "/settings/corgi";
...
function createLink(): HTMLAnchorElement {
  ...
  link.className = "nav-item-link _0_k_ui_dropdown_li";
  link.innerHTML = ICON + "Corgi";
...
function tryInject(): boolean {
  ...
  const nav =
    panel.querySelector(".cc-desk-nav") ?? panel.querySelector(".cc-app-nav");
  if (!nav) return false;
...
function observeBody(): void {
  const observer = new MutationObserver(() => {
    if (
      document.querySelector("#quickSettings") &&
