...
function getActiveCategory(): string {
  const active = document.querySelector(
    `${CATEGORY_SELECTOR} .${ACTIVE_CLASS}`,
  );
...
export const customPlaceholderPlugin = definePlugin({
  ...
  authors: ["aluminyoom"],
  description:
    "Customize the landing page search placeholder text, globally or per category",

  settings: [
    {
      key: "global",
      label: "Global placeholder (used when no category-specific text is set)",
      type: "string",
      default: "",
    },
    {
